...
static void save_dbg_stuff(struct CmdlineDebugState *dsp)
{
...
static void restore_dbg_stuff(struct CmdlineDebugState *dsp)
{
...
static CmdlineContext_T *setup_cmdline_ctx(
    char_u *cmdline,
    LineGetter fgetline,
    void *cookie,
    int flags
)
{
  ...
  CmdlineContext_T *ctx = xmalloc(sizeof(struct CmdlineContext));
  ctx->flags = flags;
  ...
  ctx->initial_msg_list = msg_list;
  msg_list = &(ctx->private_msg_list);
  ctx->private_msg_list = NULL;
  ...
  ctx->ptr_call_depth = &call_depth;
  ctx->ptr_recursive = &recursive;

  start_batch_changes();

  cstack_T *cstack = xmalloc(sizeof(cstack_T));
  cstack->cs_idx = -1;
  ...
  ctx->real_cookie = getline_cookie(fgetline, cookie);

  ctx->line_type = get_getline_equal_type(fgetline, cookie);
  // Inside a function use a higher nesting level.
  if (ctx->line_type == GETLINE_FUNC
      && ex_nesting_level == func_level(ctx->real_cookie)) {
    ex_nesting_level++;
  }
  ...
  ctx->initial_debug_stuff = setup_debug_state(flags);
  ctx->initial_trylevel = trylevel;
  ...
  if (!(flags & DOCMD_KEYTYPED) && ctx->line_type != GETLINE_EX) {
    KeyTyped = false;
  }
...
CmdlineContext_T *TEMP_setup_cmdline_ctx(
    char_u *cmdline,
    LineGetter fgetline,
    void *cookie,
    int flags,
...
CmdlineTeardown_T *cleanup_cmdline_context(
    CmdlineContext_T *ctx,
    CmdlineLoopResult_T *loop_result
)
{
  // Restore the exception environment (done after returning from the debugger)
  if (ctx->flags & DOCMD_EXCRESET) {
    restore_dbg_stuff(&(ctx->initial_debug_stuff));
  }

  CmdlineTeardown_T *result = xmalloc(sizeof(CmdlineTeardown_T));
  ...
  if (cstack.cs_idx >= 0) {
    ...
    if (!got_int
        && !current_exception
        && linegetter_missing_end(fgetline, cookie, ctx->real_cookie)) {
    ...
    do_cmdline_unwind_conditionals(&cstack);
    trylevel = ctx->initial_trylevel;
  }

  getline_type_T getline_type = ctx->line_type;
  ...
  do_errthrow(
      &cstack,
      getline_type == GETLINE_FUNC ? (char_u *)"endfunction" : (char_u *)NULL);
  ...
  if ((getline_type == GETLINE_SOURCE
       && ex_nesting_level > source_level(ctx->real_cookie))
      || (getline_type == GETLINE_FUNC
          && ex_nesting_level > func_level(ctx->real_cookie) + 1)) {
    if (!current_exception) {
      check_cstack = true;
    }
  } else {
    // When leaving a function, reduce nesting level.
    if (getline_type == GETLINE_FUNC) {
      ex_nesting_level--;
    }

    // Go to debug mode when returning from a function in which we are
    // single-stepping.
    if ((getline_type == GETLINE_SOURCE || getline_type == GETLINE_FUNC)
        && ex_nesting_level + 1 <= debug_break_level) {
      do_debug(getline_type == GETLINE_SOURCE
  ...
  if (loop_result->did_inc) {
    ...
    if (loop_result->retval == FAIL
      ...
      // doing that.
      msg_didout |= loop_result->initial_msg_didout;
      wait_return(false);
    }
  }
...
CmdlineTeardown_T *TEMP_cleanup_cmdline_context(
    CmdlineContext_T *ctx,
    CmdlineLoopResult_T *loop_result
)
{
  ...
  if (loop_result->did_inc) {
    ...
    // When just finished an ":if"-":else" which was typed, no need to wait for
    // hit-return.  Also for an error situation.
    if (loop_result->retval == FAIL
        || (did_endif && KeyTyped && !did_emsg)
        ) {
      need_wait_return = false;
  ...
  // in case do_cmdline used recursively
  did_endif = false;

  (*ctx->ptr_call_depth)--;
  end_batch_changes();

  // TODO(memory leak): Check our memory things
  // XFREE_CLEAR(ctx->cstack);
  // XFREE_CLEAR(ctx);
...
///
/// @param cmdline The current line to execute
/// @param fgetline Function to be called to get the next line to execute
/// @param cookie Passed to fgetline
/// @param flags:
...
int do_cmdline_2(char_u *cmdline,
               LineGetter fgetline,
               void *cookie,  // argument for fgetline()
               int flags)
{
  ...
  CmdlineContext_T *ctx = setup_cmdline_ctx(cmdline, fgetline, cookie, flags);
  if (ctx == NULL) {
    return FAIL;
  }

  cstack_T *cstack = ctx->cstack;
  // CmdlineLoopResult_T *loop_result = do_inner_loop(ctx);
  // CmdlineTeardownResult_T *teardown_result = cleanup_cmdline_context(...);

  // return teardown_result->retval;

  ga_init(&lines_ga, (int)sizeof(wcmd_T), 10);

  // Inside a function use a higher nesting level.
  getline_is_func = ctx->line_type == GETLINE_FUNC;

  // Get the function or script name and the address where the next breakpoint
  // line and the debug tick for a function or script are stored.
  if (getline_is_func) {
    fname = func_name(ctx->real_cookie);
    breakpoint = func_breakpoint(ctx->real_cookie);
    dbg_tick = func_dbg_tick(ctx->real_cookie);
  } else if (getline_equal(fgetline, cookie, getsourceline)) {
    fname = sourcing_name;
    breakpoint = source_breakpoint(ctx->real_cookie);
    dbg_tick = source_dbg_tick(ctx->real_cookie);
  }

  // BEGIN INNER LOOP CODE
  CmdlineLoopResult_T *loop_result = xmalloc(sizeof(CmdlineLoopResult_T));
  loop_result->did_inc = false;
  ...
  do {
    ...
    if (next_cmdline == NULL
        && !force_abort
        && cstack->cs_idx < 0
        && !(getline_is_func && func_has_abort(ctx->real_cookie))
        ) {
      did_emsg = false;
    }
    ...
    if (cstack->cs_looplevel > 0 && current_line < lines_ga.ga_len) {
      ...
      if (getline_is_func) {
        if (do_profiling == PROF_YES) {
          func_line_end(ctx->real_cookie);
        }

        if (func_has_ended(ctx->real_cookie)) {
          retval = FAIL;
          break;
        }
      } else if (do_profiling == PROF_YES
      ...
      if (do_profiling == PROF_YES) {
        if (getline_is_func) {
          func_line_start(ctx->real_cookie);
        } else if (getline_equal(fgetline, cookie, getsourceline)) {
          script_line_start();
        }
      }
    ...
    if (count++ == 0) {
       ...
       * +command file").
       */
      if (!(flags & DOCMD_NOWAIT) && !(*ctx->ptr_recursive)) {
        msg_didout_before_start = msg_didout;
        msg_didany = false;         // no output yet
        msg_start();
        msg_scroll = true;          // put messages below each other
    ...
    if (did_emsg && !force_abort
        && getline_equal(fgetline, cookie, get_func_line)
        && !func_has_abort(ctx->real_cookie))
      did_emsg = false;
  ...
  did_emsg_syntax = false;
  GA_DEEP_CLEAR(&lines_ga, wcmd_T, FREE_WCMD);
  loop_result->retval = retval;
  loop_result->initial_msg_didout = msg_didout_before_start;

  CmdlineTeardown_T *teardown_ctx = cleanup_cmdline_context(ctx, loop_result);
  ...
  XFREE_CLEAR(loop_result);

  XFREE_CLEAR(teardown_ctx);

  return retval;
}


int do_cmdline_3(LineGetter fgetline,
               void *cookie, /* argument for fgetline() */
               int flags)
{
  char_u      *next_cmdline = NULL;     /* next cmd to execute */
  char_u      *cmdline_copy = NULL;     /* copy of cmd line */
  int used_getline = FALSE;             /* used "fgetline" to obtain command */
  static int recursive = 0;             /* recursive depth */
  int msg_didout_before_start = 0;
  ...
  do {
    ...
    if (next_cmdline == NULL) {
      ...
      // Get the next line
      // TODO: This should instead do something like get a new item,
      //        then optionally send to string path.
      next_cmdline = fgetline(':', cookie, cstack.cs_idx < 0 ? 0 : (cstack.cs_idx + 1) * 2, true);
 ...
 *
 * yo, tj, it appears you can refactor this function as well.
 */
...
static inline bool sourced_lines_are_unfinished(
...
static inline bool func_lines_are_unfinished(
    LineGetter fgetline, void *cookie, void *real_cookie
)
{
...
static inline bool linegetter_missing_end(
    LineGetter fgetline, void *cookie, void *real_cookie
)
{
  return sourced_lines_are_unfinished(fgetline, cookie)
         || func_lines_are_unfinished(fgetline, cookie, real_cookie);
}
...
/// If it is in a catch clause, finish the caught exception.
/// Also cleanup any "cs_forinfo" structures.
static void do_cmdline_unwind_conditionals(cstack_T *cstack)
{
  do {
    int idx = cleanup_conditionals(cstack, 0, true);
...
static inline getline_type_T get_getline_equal_type(
    LineGetter fgetline,
    void *cookie
)
{
  if (getline_equal(fgetline, cookie, get_func_line)) {
    return GETLINE_FUNC;
  } else if (getline_equal(fgetline, cookie, getexline)) {
    return GETLINE_EX;
  } else if (getline_equal(fgetline, cookie, getexmodeline)) {
    return GETLINE_EX_MODE;
  } else if (getline_equal(fgetline, cookie, getsourceline)) {
    return GETLINE_SOURCE;
  } else {
    return GETLINE_OTHER;
  }
}

static inline void restore_cmdline_msg_list(CmdlineContext_T *ctx)
{
  assert(ctx != NULL);
  msg_list = ctx->initial_msg_list;
}

// If requested, store and reset the global values controlling the
// exception handling (used when debugging).  Otherwise clear it to avoid
// a bogus compiler warning when the optimizer uses inline functions...
static struct CmdlineDebugState setup_debug_state(int flags)
{
  struct CmdlineDebugState debug_saved;
  ...
  if (flags & DOCMD_EXCRESET) {
    ...
    memset(&debug_saved, 0, sizeof(struct CmdlineDebugState));
  }
